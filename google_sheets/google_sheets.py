...
class Sheet:
    ...
    def insert_row(self, row_cells: list[Cell], row_num: int) -> None:

        row_data = Row(row_cells, self.sheet_id, row_num + 1)  # +1 for header
...
class GoogleSheets:
    ...
    def write(self) -> None:
        ...
        self.service.spreadsheets().batchUpdate(spreadsheetId=self.spreadsheet_id, body=self.get_request_body()).execute()
